...
// OffsetResolver represents a function that can resolve the value of a given offset
type OffsetResolver func(int) int

// SimpleIncrementer is an OffsetResolver function that increase the offset by 1
func SimpleIncrementer(offset int) int {
	return offset + 1
}

// BiasedDecrementer is an OffsetResolver function that decrements the offset by 1 if the offset
// is 3 or more, and increments by 1 otherwise
...
// Trace processes the instructions in the Stack and returns the number of instructions processed
